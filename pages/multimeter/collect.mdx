...
Multimeter toggle has to be in the correct position (when applicable) before you start capturing. Capture begins When a notification at the top of the screen appears. Captured values will automatically populate selected field or fields (in case of ON/OFF potential capture).

To save measurement, press button on the left side of the selected field one more time.

<Callout type="info" emoji="ℹ️">
 Pokit Pro supports capturing reading when physical button is pressed, however there is a bug that causes multimeter to restart after button is pressed when measuring ON/OFF potentials.
