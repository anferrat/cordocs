...
<Callout type="info" emoji="ℹ️">
  When exporting to a spreadsheet, each row will correspond with a single survey
  item.
</Callout>
...
- **Images** - If you select to export images, the survey images will be exported to a .zip archive. This option is only available for premium users.
...
- **Item properties** - choose the item properties that you wish to export. A column in the spreadsheet will be added for each selected property.
...
<Callout>
  **EXAMPLE:** You have a test point that has one pipeline test lead with ON/OFF
  ...
  potentials in the exported list, you must select pipeline test lead and anode
...
<Callout>
  **EXAMPLE:** Let's consider two test points – TP1 and TP2. TP1 is a test
  ...
  PIPE_TEST_LEAD_OFF, RISER_ON, and RISER_OFF. However, if both the riser and
  pipeline test lead are assigned to the same pipeline (e.g., WATER_FEED), and
